...
std::pair<std::pair<int, int>, std::pair<int, int>> generateKeyPair() {
    // Choose two large prime numbers, p and q
    int p = 23;  // Replace with actual prime numbers
    int q = 29;  // Replace with actual prime numbers
    ...
    int e = 31;  // Replace with actual public exponent
...
std::vector<int> encryptMessage(const std::string& message, const std::pair<int, int>& publicKey) {
...
std::string decryptMessage(const std::vector<int>& ciphertext, const std::pair<int, int>& privateKey) {
...
int main() {
    ...
    // Example message
    std::string originalMessage = "parola";

    // Encrypt the message with the public key
    auto encryptedMessage = encryptMessage(originalMessage, keyPair.first);

    // Decrypt the message with the private key
    std::string decryptedMessage = decryptMessage(encryptedMessage, keyPair.second);
